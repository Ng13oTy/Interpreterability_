...
import numpy as np
from My_Explainer.PGExplainer import PGExplainer
...
for m_type in model_types:
    ...
    for i in range(1):
        # 每种解释方法解释十次
        for exp_time in range(1):
            print('current model:{}, current explain time {}'.format(i, exp_time))
            ...
            explainer = PGExplainer(epochs=epochs)
            # 训练过程
            ...
            for epoch in range(epochs):
                for idx in correct_idx:
                    ...
                    explainer.train(epoch, model, feas[idx].detach(), graphs_[idx].detach(), gt_label)
            ...
            with open('result/{}/{}_best_model_{}_pge_{}.csv'.format(m_type, m_type, i, exp_time),
                      'w', newline='') as f:
                writer = csv.writer(f)
